...
var passwordCheckOld = function () {
    ...
    // Checks to see if the password is at least 8 characters. If not, it prompts the user to re-enter. 
    if (stringPassword.length < 8) {
        stringOutput = "The entered password is not greater than 8 characters. Please re-enter.";
    }
    // If the password is the correct length, the for loop is entered to check whether or not it matches on of the 'bad' passwords. 
    else {
        for (var i = 0; i < arrayBadPasswords.length; i++) {
            // If there's a match, stringOutput is set to an warning and the loop is broken.
            if (stringPassword === arrayBadPasswords[i]) {
                stringOutput = "The password entered matches one of the 'bad' passwords from the list. Please re-enter.";
                break;
            }
            // If there is not a match, the password is deemed secure and the user is notified. 
            else {
                stringOutput = "Congrats, your password is secure!";
            }
        }
    }
    //stores the value of stringOutput in the DOM. 
