...
if (boolPrinting === true)
{
    if (boolRedLight === true)
    {
        if (boolRecognised === true)
     {
        ...
        else //boolPrinting = true, boolRedLight = true, boolRecognized = false (TTF)
        {
            stringOutput = stringMessage4 + stringMessage5;
        }
    }
    else if (boolRecognised === true) //boolPrinting = true, boolRedlight = false, boolRecognized = true  (TFT)
    {
        stringOutput = stringMessage1 + stringMessage2 + stringMessage3;
    }
    else //boolPrinting = true, boolRedlight = false, boolRecognized = false (TFF)
        {
            stringOutput = stringMessage5;
        }
    }
else if (boolRedLight === true) //Occurs if the user inputs that the printer IS printing (boolPrinting = false) (FTT)
    {
        if (boolRecognised === true)
        {
            stringOutput = stringMessage3 + stringMessage4;
        }
        else //boolPrinting = false, boolRedLight = true, boolRecognized = false (FTF)
        {
            stringOutput = stringMessage4;
        }
    }
    else if (boolRecognised === true) //boolPrinting = false, boolRedLight = false, boolRecognized = true (FFT)
    {
        stringOutput = stringMessage3;
    }
    else //boolPrinting = false, boolRedLight = false, boolRecognized = false (FFF) > Working OK!
    {
